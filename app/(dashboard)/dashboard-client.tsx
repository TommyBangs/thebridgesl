...
interface DashboardClientProps {
    trendingSkills: any[] // Replace with proper types
    opportunities: any[]
}

export default function DashboardClient({ trendingSkills, opportunities }: DashboardClientProps) {
                ...
                <div className="lg:col-span-2 space-y-6">
                    <section>
                        ...
                        <div className="grid gap-4 sm:grid-cols-2">
                            {trendingSkills.slice(0, 6).map((item) => (
                                // Handle structure difference: service returns { skill: {...}, ... } but UI expects skill object directly or mapped
                                // Adjusting mapping based on service return: getTrendingSkills returns TrendingSkill[] which includes skill relation
                                <TrendingSkillCard key={item.skillId} skill={{
                                    ...item.skill,
                                    trending: {
                                        demandPercentage: Number(item.demandPercentage),
                                        growthRate: Number(item.growthRate),
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            {opportunities.map((opportunity) => (
                                // Map Prisma opportunity to UI component props if needed
                                <OpportunityCard key={opportunity.id} opportunity={{
                                    ...opportunity,
                                    matchScore: 0, // Placeholder as calculation logic is not yet in service
                                    salary: { min: Number(opportunity.salaryMin), max: Number(opportunity.salaryMax), currency: "SLL" },
                                    postedDate: opportunity.postedDate.toISOString(),
                                    skills: opportunity.skills.map((s: any) => s.skill.name)
                                }} />
                            ))}
                        </div>
                    </section>
